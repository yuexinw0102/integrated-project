...
class BaseDao {
  ...
  findOne({ columns, where, table } = {}, value) {
    const sql = `select ${columns ? columns : " * "} 
  ...
  // 根据分页条件查询
  findByPage(
    { columns, where, table, group_by, having, order_by } = {},
    value
  ) {
    const sql = `select${columns ? columns : " * "}from ${table ? table : this.table}${where ? where : ""}${group_by ? group_by : ""}${having ? having : ""}${order_by ? "order by" + order_by : "order by " + this.primaryKey} `;
    return this.queryWithPromise(sql, value);
  }
  ...
  findAll({ columns, where, table, order_by } = {}, value) {
    const sql = `select${columns ? columns : " * "}from ${
