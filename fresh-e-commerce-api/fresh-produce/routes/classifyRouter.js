...
const dao = require('../dao/classifyDao.js');
...
// 定义添加/编辑验证规则
const rules = [
  body('classifyName').trim().exists().withMessage('classifyName属性必填').bail(),
  body('tier').trim().exists().withMessage('tier属性必填').bail(),
  body('superior').trim().exists().withMessage('superior属性必填').bail(),
